...
let materialVerso = new THREE.MeshBasicMaterial({
    ...
    map: loader.load('img/card-verso.jpeg'),
});

let geometry = new THREE.PlaneGeometry(8, 12, 32);

var asap = new Howl({
    src: ['./sounds/1_AsapRocky.mp3'],
    sprite: {
        sample: [19000, 15000]
    }
});
...
data.forEach((item) => {
    ...
    let materialRecto = new THREE.MeshBasicMaterial({
        side: THREE.BackSide,
        map: loader.load(`img/card-recto-${item.id}.jpeg`),
    });

    console.log(materialRecto)
...
function initCameraPosition(){
    ...
    TweenMax.to(camera.position, 1, {z: 27});
    TweenMax.to(cards.rotation, 1, {z: -.8});
}
...
function showCard(object) {
    ...
    if (cardVisible.length < 2 && !object.parent.data.valid) {
        ...
        TweenMax.to(object.parent.rotation, .5, {
            y: Math.PI, onStart: ()=> {
