    ...
    constructor(context,url,analyser) {
        ...
        function onLoadSound(data) {
          ...
          function onSuccess(buffer) {
            soundBuffer = buffer;
            this.this.source = context.createBufferSource(); // Creates a sound source
            this.source.buffer = soundBuffer; // Source to play
            this.source.connect(gainNode); //Connexion au enceinte
            gainNode.gain.value = 1;
            gainNode.connect(context.destination); //Connexion au enceinte
            this.source.connect(analyser); // relier a l'analyser sur une autre branche
            this.source.start(0);
          }
          function onError() {
