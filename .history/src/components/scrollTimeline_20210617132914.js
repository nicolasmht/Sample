...
import soundD from '../audios/tundra-beats.mp3'
...
function ScrollTimeline(scene, camera) {
        ...
        let sound1 = new Howl({src: [soundA]});
        let sound2 = new Howl({src: [soundB]});
        let sound3 = new Howl({src: [soundC]});
        let sound4 = new Howl({src: [soundC]});
        let sound5 = new Howl({src: [soundC]});
        let sound6 = new Howl({src: [soundC]});
    ...
    function initTimeline() {
        ...
        .add(()=> { console.log('Step 1 t:1')
            sound1.seek(0);
            sound1.play();
            sound1.fade(0, 1,1300);
        },1) // STEP 1
        .add(()=> { console.log('Step 2 t:1.75')
            playSound(sound1, sound2)
    ...
    function playSound(soundOne, soundTwo) {
        soundOne.once( 'fade', () => { soundOne.stop(); });
        soundOne.fade( soundOne.volume(), 0, 1000 );

        soundTwo.seek(0);
        soundTwo.play();
        soundTwo.fade(0, 1,1300);
    }
    ...
    function initSlider() {
        ...
        function onPointerMove( e ) {
            ...
            sliderPos = Math.max( 0, Math.min( window.innerHeight, e.pageY ) );

            slider.style.left = sliderPos - ( slider.offsetHeight / 2 ) + "px";

            console.log((sliderPos - slider.offsetHeight)/1000);
            console.log(((1000 - sliderPos) - slider.offsetHeight)/1000);

            sound02.volume((sliderPos - slider.offsetHeight)/1000);
            sound01.volume(((1000 - sliderPos) - slider.offsetHeight)/1000);
