    ...
    for(let i=1; i<7; i++) {
        document.querySelector('.cta-'+i).addEventListener('click',()=>{
            if(isClosed) {
                ...
                setTimeout(()=>{document.querySelector('.cross-'+i).classList.add('cross-visible')},1000)
                isClosed = false;
            } else {
                document.querySelector('.container_artist-panel-'+i).style.transform = 'translateX(0%)';
                setTimeout(()=>{document.querySelector('.cross-'+i).classList.remove('cross-visible')},1000)
                isClosed = true;
            }
        })
