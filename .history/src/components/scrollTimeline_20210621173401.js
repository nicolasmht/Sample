...
function ScrollTimeline(scene, camera) {
        ...
        textureCaseLoad.repeat.set( 0.1, 0.1 );
        ...
        loader.load( TapeModel, ( gltf ) => {
            ...
            tape.traverse( (child) => {
                ...
                switch(modelPart) {
                    ...
                    case 'desk_haut2':
                    case 'desk_haut3':
                        child.material = new THREE.MeshBasicMaterial({color:0xEBD3BA, map: textureCaseLoad, side:THREE.DoubleSide});
                    break;
                    case 'Tape_obj':
            ...
            tape.position.set(camera.position.x -0.0115 , camera.position.y -0.085, camera.position.z -0.2004);
