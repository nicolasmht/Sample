...
function ScrollTimeline(scene) {
    ...
    function initTimeline() {
        ...
        let tape = tape.getObjectByName('Tape')
        ...
        .add(()=> {
            var texture = textureLoader.load(TapeTexture);
            tape.children[0].material = new THREE.MeshPhongMaterial({map: texture});
        },2)
        .add(()=> {
            var texture2 = textureLoader.load(TapeTexture2);
            tape.children[0].material = new THREE.MeshPhongMaterial({map: texture2});
        },2)
        .to(tape.position, 2, {x: 14})
