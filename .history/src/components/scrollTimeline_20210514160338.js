...
import TapeModel from '../objects/AudioTape/TapeCaseVerticalStorageShader.glb';
...
function ScrollTimeline(scene) {
    ...
    setTimeout(()=>{ if(tape) console.log(tape) },2000)
    ...
    function initTimeline() {
        ...
        // .to(".box", 2, {x:'500px'})
        let hola = tape.getObjectByName('Tape_obj')


        var timelineTape = new TimelineMax({ paused: true })
        .to(hola.position, 2, {x: 2,y: -13},0)
        .to(hola.rotation, 2, {x: 8, y:7}, 0)
        .to(hola.rotation, 2, {x: 1.5, y:0, z: 1.5}, 1)
        .to(hola.rotation, 5, {z: 25}, 2)
        .add(()=> {
            var texture = textureLoader.load(TapeTexture);
            hola.children[0].material = new THREE.MeshPhongMaterial({map: texture});
        },2)
        .add(()=> {
            var texture2 = textureLoader.load(TapeTexture2);
            hola.children[0].material = new THREE.MeshPhongMaterial({map: texture2});
        },2)
        .to(hola.position, 2, {x: 14})
