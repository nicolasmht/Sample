...
function ScrollTimeline(scene) {
    ...
    function initTimeline() {
        ...
        let caseB = tape.getObjectByName('Case_b')


        var timelineTape = new TimelineMax({ paused: true })
        .to(tapeGroup.position, 2, {x: 2,y: -13},0)
        .to(caseB.rotation, 2, {x: -2},0)
        .to(tapeGroup.rotation, 2, {x: 8, y:7}, 0)
        .to(tapeGroup.rotation, 2, {x: 1.5, y:0, z: 1.5}, 1)
        .to(tapeGroup.rotation, 5, {z: 25}, 2)
        .add(()=> {
            var texture = textureLoader.load(TapeTexture);
            tapeGroup.children[0].material = new THREE.MeshPhongMaterial({map: texture});
        },2)
        .add(()=> {
            var texture2 = textureLoader.load(TapeTexture2);
            tapeGroup.children[0].material = new THREE.MeshPhongMaterial({map: texture2});
        },2)
        .to(tapeGroup.position, 2, {x: 14})
