...
function ScrollTimeline(scene, camera) {
    ...
    document.querySelector('.cta-one').addEventListener('click',()=>{
        console.log('hola')
        if(isClosed) {
            document.querySelector('.container_artist-panel-one').style.transform = 'translateX(-100%)'
            isClosed = false;
        } else {
            document.querySelector('.container_artist-panel-one').style.transform = 'translateX(0%)'
            isClosed = true;
        }
    })
