...
function DaftPunk(sceneMain, cameraMain, interactionManagerMain) {
    ...
    var render = function () {
        ...
        if(frequencyData[0]/20 < 4) {
            ...
            // gsap.to(pointLight, .15, {intensity: 2})
        }
        if(frequencyData[0]/20 > 4 && frequencyData[0]/20 < 6) {
            pointLight.intensity = 6
            // gsap.to(pointLight, .15, {intensity: 4})
        }
        if(frequencyData[0]/20 > 6 && frequencyData[0]/20 < 7) {
            pointLight.intensity = 12
            // gsap.to(pointLight, .15, {intensity: 8})
        }
        if(frequencyData[0]/20 > 7 && frequencyData[0]/20 < 9) {
            pointLight.intensity = 18
            // gsap.to(pointLight, .15, {intensity: 15})
        }
        if(frequencyData[0]/20 > 9 ) {
            pointLight.intensity = 30
            // gsap.to(pointLight, .15, {intensity: 22})
        }
