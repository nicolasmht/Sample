...
function mergeMesh(mesh) {
  ...
  mesh.traverse((m) => {
    if(m.material) {
      m.material = new MeshBasicMaterial({color: 0xff00ff});
    }
    if (m instanceof Mesh) {
...
async function loadSuzanne() {
  const model = await loadModel("../assets/cabMin.obj");
  const geo = mergeMesh(model);
  const modified = new SubdivisionModifier(0);
...
async function generate() {
  ...
  console.log('suz',suzanne);
  suzanne.material = new MeshBasicMaterial({color: 0xff00ff});
  suzanne.material.transparent = true
  // suzanne.material.opacity = 0.5
  group.add(suzanne);
}
